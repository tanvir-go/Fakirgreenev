            ...
            {packages.map((pkg) => (
              <Card key={pkg.title} className="flex flex-col">
                <CardHeader>
                  ...
                  <CardTitle className="flex items-center gap-2"><Star className="text-gradient"/>{pkg.title}</CardTitle>
                  <CardDescription>{pkg.duration} | Starting from <span className="font-bold text-gradient">{pkg.price}</span></CardDescription>
                </CardHeader>
